...
const TransportHome = () => {
  ...
  const fetchHarvests = async () => {
    try {
      const response = await axios.get('http://localhost:5000/h/transportdata');
  ...
  const columns = [
    { field: 'transportId', headerName: 'Transport ID', width: 150 },
    ...
    { field: 'Date', headerName: 'Date', width: 150 },
    {
      ...
      renderCell: (params) => (
        <Button
          variant="contained"
          color="primary"
          onClick={() => navigate(`/Transport-product-view/${harvests[params.id-1]._id}`)}
        >
          View
        </Button>
      ),
